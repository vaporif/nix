#!/usr/bin/env bash
# Notify when Claude needs input: macOS notification + phone push via ntfy.sh
...
NTFY_TOPIC=$(cat /run/secrets/ntfy-topic 2>/dev/null) || true
if [[ -n "$NTFY_TOPIC" ]]; then
  curl -sf -o /dev/null -d "$message" -H "Title: $title" -H "Priority: high" "ntfy.sh/$NTFY_TOPIC" &
fi
